...
export const Information = ({ isOpen, onOpenChange }: any) => {
  const [headers, setHeaders] = useState<string[]>(['Accept: application/json', 'Content-Type: application/json']);
                    ...
                    <form className="flex flex-col gap-2">
                      {
                        headers.map((header, i) => (
                          <Input label="Header" placeholder="Accept: application/json" defaultValue={header} key={i} />
                        ))
                      }
